...
export default function EditSoftPlayItem({ item }: { item: SoftPlayItem }) {
    const [icons, setIcons] = React.useState<{ src: string; label: string }[]>(item.icons || []);
    const [selectedFiles, setSelectedFiles] = React.useState<File[]>([]);
    const [selectedVideos, setSelectedVideos] = React.useState<File[]>([]);
    const [existingImages, setExistingImages] = React.useState<string[]>(item.images || []);
    const [existingVideos, setExistingVideos] = React.useState<string[]>(item.videos || []);
    const [mainImage, setMainImage] = React.useState<string>(item.main_image || '');
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <div>
                    <h1 className="text-2xl font-semibold">Edit Soft Play Item</h1>
                    <p className="text-sm text-muted-foreground">Update the item information</p>
                </div>

                <Card className="max-w-2xl">
                    <CardHeader>
                        <CardTitle>Item Details</CardTitle>
                        <CardDescription>Modify the soft play item information</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Form
                            action={`/dashboard/soft-play/${item.id}`}
                            method="post"
                        ...
                        >
                            {({ processing, errors }) => (
                                <>
                                    ...
                                    <div className="grid gap-2">
                                        <Label htmlFor="videos">Add More Videos</Label>
                                        <Input
                                            ref={videoInputRef}
                                            id="videos"
                                            name="videos[]"
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Select
                                            name="is_active"
                                            defaultValue={item.is_active ? '1' : '0'}
                                        >
                                            <SelectTrigger>
                                                <SelectValue />
                                            </SelectTrigger>
